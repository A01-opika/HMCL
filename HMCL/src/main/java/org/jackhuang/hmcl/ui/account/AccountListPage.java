...
public final class AccountListPage extends DecoratorAnimatedPage implements DecoratorPage {
    ...
    private final ObservableList<AccountListItem> filteredItems;
    ...
    public AccountListPage() {
        ...
        selectedAccount = createSelectedItemPropertyFor(filteredItems, Account.class);
        
        // Initialize filtered items
        updateFilteredItems();
        
        // Listen to search keyword changes
        searchKeyword.addListener((observable, oldValue, newValue) -> updateFilteredItems());
        
        // Listen to accounts changes
        accounts.addListener((javafx.collections.ListChangeListener<Account>) change -> updateFilteredItems());
    }
        ...
        filteredItems.setAll(
            items.stream()
                .filter(item -> {
                    ...
                    String character = account.getCharacter().toLowerCase();
                    String username = account.getUsername().toLowerCase();
                    return character.contains(keyword) || username.contains(keyword);
                })
    ...
    public ObservableList<AccountListItem> getFilteredItems() {
    ...
    private static class AccountListPageSkin extends DecoratorAnimatedPageSkin<AccountListPage> {
        ...
        public AccountListPageSkin(AccountListPage skinnable) {
            ...
            VBox list = new VBox();
            {
                ...
                container.maxWidthProperty().bind(scrollPane.widthProperty());
                container.setSpacing(10);

                // Search box
                TextField searchField = new TextField();
                searchField.setPromptText(i18n("account.search"));
                searchField.textProperty().bindBidirectional(skinnable.searchKeywordProperty());
                ...
                Bindings.bindContent(list.getChildren(), skinnable.getFilteredItems());

                // Enable drag and drop for reordering
                setupDragAndDrop(list, skinnable);

                container.getChildren().addAll(searchField, list);
        ...
        private void setupDragAndDrop(VBox list, AccountListPage skinnable) {
            ...
            list.setOnDragDropped((DragEvent event) -> {
                ...
                if (db.hasString()) {
                    ...
                    for (int i = 0; i < accounts.size(); i++) {
                        if (accounts.get(i).getIdentifier().equals(accountId)) {
                            draggedAccount = accounts.get(i);
                            sourceIndex = i;
                            break;
                        }
                    }
                    ...
                    if (draggedAccount != null && sourceIndex != targetIndex) {
                        ...
                        accounts.remove(sourceIndex);
                        // Insert at new position
                        int newIndex = targetIndex > sourceIndex ? targetIndex - 1 : targetIndex;
                        if (newIndex < 0) newIndex = 0;
                        if (newIndex > accounts.size()) newIndex = accounts.size();
                        accounts.add(newIndex, draggedAccount);
                        success = true;
                    }
                }
